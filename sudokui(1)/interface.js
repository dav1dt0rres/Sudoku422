...
function setupgame(seed) {
  ...
  if (loadgame(storagename(seed))) {
    console.log("Setting up saved game")
    return; }
...
function checkifValid(state_1){
 // console.log("Inside of check Valid")
  var state = currentstate();
  var sofar = boardsofar(state_1);
  // Check for conflicts.
  var conflicts = SudokuHint.conflicts(sofar);
  console.log("length",conflicts.length)
  if (conflicts.length == 0) {
    ...
    for (var i=0;i<conflicts.length;++i){
     console.log("Printing Errors",conflicts[i].errors);
    }
...
$(document).on('mousedown', 'td.sudoku-cell', function(ev) {
  ...
  console.log("Importing current number"+num)
  if (num == -1) {
    // Erase this square.
    ...
    if(checkifValid(state).length==0){
      ...
      console.log("loggin an Error!")
      $(document).trigger('log', ['error',{}]);
...
function ClearBackground(){
  for(var i=0;i<16;++i){
    document.getElementById('sc'+i.toString()).style.backgroundColor = "white";
  }
}
function setcurnumber(num) {
  ...
  if (num!=0){
    for(var i=0;i<16;++i){//i is the position on the square
      //var pos = parseInt($('#sc0').attr('id').substr(2));
      var state = currentstate();
     var temp=state.answer[i]
        state.answer[i]=num-1;
        console.log("Checking at index"+" "+i+" "+state.answer[i])
      if(checkifValid(state).length==0 && state.puzzle[i]==null && temp==null ){

        console.log("Should be highlighted"+" "+'#sc'+i.toString())
        document.getElementById('sc'+i.toString()).style.backgroundColor = "green";

        //$('#sc'+i.toString()).trigger('mouseenter');
  ...
  console.log("Current Number after selection"+" "+num)
  curnumber = num;
}
